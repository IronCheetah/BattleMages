...
key_jetpack = mouse_check_button(mb_right) || gamepad_button_check(1, gp_face2);
key_fireball = mouse_check_button(mb_left) || gamepad_button_check(1, gp_face4);
...
//Animation
	...
	if (grounded)
	{
		...
		else 
		{
			if (hsp = 0)
			{
			sprite_index = blue_mage_idle;
			}
			else
			{
			if (sprite_index!= blue_mage_run)image_index = 0;
			 sprite_index = blue_mage_run;
			}
	}
	}
